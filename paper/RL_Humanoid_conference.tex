...
% Tighter float spacing for the camera-ready page budget
...
\begin{itemize}
    ...
    \item Experimental evidence that episode-termination logic can determine learning success on uneven terrain: controlled ablations show that fixed world-frame health bounds can prematurely terminate healthy agents on elevated terrain, while terrain-relative termination enables successful stair traversal and circuit completion, reaching up to 87\% completion on the combined navigation-and-stairs task.
    \item A reproducible evaluation methodology based on 100-episode success-rate evaluations for every task, demonstrating up to 96\% stair-climbing success and providing openly available environments, training configurations, and evaluation scripts for future comparison and benchmarking.
\end{itemize}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.45\linewidth]{images/action_space.png}
    \caption{Action space: the 17 torque-controlled joints of the MuJoCo Humanoid.}
    \label{fig:action_space}
\end{figure} All environments use a physics timestep of $0.003$\,s with frame skip 5, i.e., a control interval of $dt = 0.015$\,s ($\approx 66.7$\,Hz).
...
\end{equation}
where $y_t$ is the lateral offset of Section~\ref{sec:robot_model}, penalizing world-frame drift from the corridor centerline $y{=}0$. Table~\ref{tab:reward_terms} collects every weight and task-specific value of this section; the prose below only defines the terms.
...
\end{multline}
with progress $r_t^{\mathrm{prog}} = w_p (d_{t-1} - d_t)$, where $d_t$ is the planar distance to the active waypoint; directional velocity $r_t^{\mathrm{dir}} = w_f \max(0, \mathbf{v}_t \!\cdot\! \hat{\mathbf{u}}_t)$, projecting the planar velocity of Section~\ref{sec:robot_model} onto the unit direction $\hat{\mathbf{u}}_t$ to the waypoint; heading alignment $r_t^{\mathrm{head}} = w_\psi \cos(\epsilon_t) \min(\lVert\mathbf{v}_t\rVert, 2)$, which for velocity aligned with the heading equals the saturated velocity component toward the waypoint (both terms active only beyond $0.1$\,m of the waypoint); and sparse bonuses $r_t^{\mathrm{bonus}} = b_{\mathrm{wp}} \mathbb{I}[d_t < \tau] + b_{\mathrm{circ}} \mathbb{I}[\text{all waypoints reached}]$ for entering the active waypoint's reach radius $\tau$ and for completing the course. Two regularizers stabilize multi-objective learning:
\begin{equation}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.70\textwidth]{figures/combined_training_curves.pdf}
...
\begin{table}[t]
\centering
\caption{Evaluation-time termination ablation: trained S2/S4 policies re-evaluated for 100 episodes under each rule. World-frame bounds fire on elevated terrain, truncating S2 well short of a fall.}
